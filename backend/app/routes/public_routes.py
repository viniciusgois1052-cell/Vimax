...
def get_ativos_empresa(empresa_id):
    """Retorna os ativos ativos de uma empresa."""
    # ✅ Filtrar apenas ativos sem data_inativacao (ativos em uso)
    ativos = Ativo.query.filter_by(empresa_id=empresa_id).filter(
        Ativo.data_inativacao == None
    ).all()
...
def abrir_chamado_portal():
    ...
    Herda localizacao_id, fornecedor_id, contrato_id e orcamento_id
    do ativo ou infraestrutura selecionado buscando direto no banco.
    ...
    # ✅ Herdar todos os vínculos do ativo ou infraestrutura buscando no banco
    localizacao_id = None
    fornecedor_id = None
    contrato_id = None
    orcamento_id = None
 
    if tipo == 'maquinario' and data.get('ativo_id'):
        ativo = Ativo.query.get(int(data.get('ativo_id')))
        if ativo:
            localizacao_id = ativo.localizacao_id
            fornecedor_id  = ativo.fornecedor_id
            contrato_id    = ativo.contrato_id
            orcamento_id   = ativo.orcamento_id
 
    elif tipo == 'infraestrutura' and data.get('infraestrutura_id'):
        infra = Infraestrutura.query.get(int(data.get('infraestrutura_id')))
        if infra:
            localizacao_id = infra.localizacao_id
 
    # Processar opcoes_selecionadas
    opcoes = data.get('opcoes_selecionadas', [])
    opcoes_json = json.dumps(opcoes) if opcoes else None
 
    # Salvar fotos fisicamente no servidor
