...
def list_infraestruturas():
    ...
    # ✅ Se per_page não for enviado, retorna TODOS (sem paginação)
    per_page_raw = request.args.get('per_page')
    use_pagination = per_page_raw is not None
    page = int(request.args.get('page', 1))
    per_page = int(per_page_raw) if use_pagination else None
    ...
    total = query.count()
    query = query.order_by(desc(Infraestrutura.created_at))

    if use_pagination:
        itens = query.offset((page - 1) * per_page).limit(per_page).all()
    else:
        itens = query.all()  # ← retorna tudo
