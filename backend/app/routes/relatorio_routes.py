...
def dashboard_relatorios():
    try:
        ...
        # 2. Custos de Chamados por Ativo (top 10)
        ...
        # 3. Gastos de Orçamentos por Ativo (top 10)
        # ✅ CORREÇÃO: join correto via Ativo.orcamento_id -> Orcamento.id
        # e sem filtro de status para não perder dados — exibe todos os orçamentos vinculados
        ...
        # 4. Ativos sem contrato — total global
        ativos_sem_contrato_count = (
            db.session.query(func.count(Ativo.id))
            .filter(Ativo.contrato_id == None)
            .scalar() or 0
        )
 
        # 5. Ativos sem contrato — detalhes (com join correto para localização)
        # ✅ CORREÇÃO: usar join com Localizacao em vez de Ativo.localizacao (não é coluna)
        ativos_sem_contrato_detalhes_raw = (
            db.session.query(Ativo.nome, Empresa.nome, Localizacao.nome)
            .join(Empresa, Ativo.empresa_id == Empresa.id)
            .outerjoin(Localizacao, Ativo.localizacao_id == Localizacao.id)
            .filter(Ativo.contrato_id == None)
            .order_by(Empresa.nome, Ativo.nome)
            .all()
        )
        ativos_sem_contrato_detalhes = [
            [a[0], a[1], a[2] or 'Não informada']
            for a in ativos_sem_contrato_detalhes_raw
        ]
 
        # 6. Gasto médio global (Orçamentos)
        gasto_medio_global = (
            db.session.query(func.avg(Orcamento.valor))
            .filter(Orcamento.status == 'Aprovado')
            .scalar() or 0
        )
 
        # 7. Chamados por categoria
        categorias_count = (
        ...
        total_gasto_orcamentos = (
            db.session.query(func.sum(Orcamento.valor))
            .filter(Orcamento.status == 'Aprovado')
            .scalar() or 0
        )
        total_custo_chamados = (
...
def export_empresas_stats():
    try:
        ...
        ws.append(["Empresa", "Qtd. Ativos", "Ativos sem Contrato", "Custo Chamados", "Gasto Orçamentos"])
        for e in empresas_stats:
            ws.append([e["nome"], e["ativos"], e["ativos_sem_contrato"], e["custo_chamados"], e["gasto_orcamentos"]])
...
def export_custos_ativos_chamados():
    try:
        ...
        ws.append(["Ativo", "Custo Total Chamados"])
...
def export_gastos_ativos_orcamentos():
    try:
        # ✅ CORREÇÃO: join correto e sem filtro de status
        ativos_gastos_orc = (
            db.session.query(Ativo.nome, func.sum(Orcamento.valor))
            .join(Orcamento, Ativo.orcamento_id == Orcamento.id)
            .group_by(Ativo.nome)
        ...
        ws.append(["Ativo", "Gasto Total Orçamentos"])
...
def export_chamados_por_categoria():
    try:
        ...
        ws.append(["Categoria", "Qtd. Chamados"])
...
def export_chamados_por_mes():
    try:
        ...
        ws.append(["Mês/Ano", "Qtd. Chamados"])
...
def export_ativos_sem_contrato():
    try:
        # ✅ CORREÇÃO: join com Localizacao em vez de Ativo.localizacao (não é coluna)
        ativos_sem_contrato_detalhes = (
            db.session.query(Ativo.nome, Empresa.nome, Localizacao.nome)
            .join(Empresa, Ativo.empresa_id == Empresa.id)
            .outerjoin(Localizacao, Ativo.localizacao_id == Localizacao.id)
        ...
        ws.append(["Ativo", "Empresa", "Localização"])
        for a in ativos_sem_contrato_detalhes:
            ws.append([a[0], a[1], a[2] or 'Não informada'])
