...
ROOT = Path.cwd()
MODEL = ROOT / "app" / "models" / "fornecedor_avaliacao.py"
ROUTE = ROOT / "app" / "routes" / "fornecedor_avaliacao_routes.py"
...
if not MODEL.exists() or not ROUTE.exists():
    fail(
        "execute este script em /var/www/cmms_project/backend "
        "após instalar o backend de classificação"
    )
...
old_order_column = '''    ordem_compra_id = db.Column(
        db.Integer,
        db.ForeignKey('ordens_compra.id', ondelete='RESTRICT'),
        nullable=False
    )'''
new_order_column = '''    ordem_compra_id = db.Column(
        db.Integer,
        db.ForeignKey('ordens_compra.id', ondelete='RESTRICT'),
        nullable=True
    )'''

if old_order_column in model_original:
    model_new = model_original.replace(
        old_order_column,
        new_order_column,
        1
    )
elif new_order_column in model_original:
    model_new = model_original
else:
    fail(
        "não encontrei a coluna ordem_compra_id no modelo; "
        "nenhum arquivo foi alterado"
    )


model_backup = MODEL.with_name(
    MODEL.name + ".bak-simples-" + STAMP
)
route_backup = ROUTE.with_name(
    ROUTE.name + ".bak-simples-" + STAMP
)
shutil.copy2(MODEL, model_backup)
shutil.copy2(ROUTE, route_backup)
print("Backup criado: {}".format(model_backup))
print("Backup criado: {}".format(route_backup))


try:
    # Torna a OC opcional sem apagar tabela nem avaliações.
    migration = subprocess.run(
        [
            ...
            (
                ...
                "'ALTER TABLE fornecedor_avaliacoes "
                "MODIFY COLUMN ordem_compra_id INT NULL'));"
    ...
    if migration.returncode != 0:
        raise RuntimeError(
            ...
            or migration.stdout.strip()
            or "falha ao alterar a tabela"
        )

    MODEL.write_text(model_new, encoding="utf-8")
    ...
    shutil.copy2(route_backup, ROUTE)
    print("ERRO: {}".format(exc))
    print("Arquivos restaurados automaticamente.")
    print(
        "A coluna eventualmente alterada para NULL é compatível "
        "com o modelo anterior e não remove dados."
    )
    sys.exit(1)
...
print()
print("=" * 60)
print("CLASSIFICAÇÃO SIMPLES APLICADA")
print("=" * 60)
print("Nota: 1 a 5 estrelas.")
print("Comentário: opcional.")
print("Ordem de Compra: não obrigatória.")
print("Permissão para avaliar: compras_editar.")
print("Nenhuma tabela ou avaliação foi apagada.")