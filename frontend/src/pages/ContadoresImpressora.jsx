...
export default function ContadoresImpressora() {
  ...
  const openEdit = (c) => {
    ...
    setFormData({
      ...
      http_usuario: c.http_usuario || '',
      http_senha: c.http_senha || '',
      http_porta: c.http_porta ? String(c.http_porta) : '',
    })
    setIsFormOpen(true)
  }
  ...
  const saveForm = async () => {
    ...
    const payload = {
      ...
      http_usuario: formData.http_usuario?.trim() || null,
      http_senha: formData.http_senha?.trim() || null,
      http_porta: formData.http_porta ? parseInt(formData.http_porta, 10) : null,
    }
          ...
          <div className="md:col-span-2">
            <div className="border-t border-slate-100 pt-3 mt-1">
              <p className="text-[10px] uppercase tracking-widest text-slate-400 font-black mb-3">
                🔐 Autenticação HTTP — opcional, apenas se a impressora exigir login na interface web
              </p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-black text-slate-600 uppercase mb-1">Usuário Web</label>
                  <input
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-primary"
                    value={formData.http_usuario}
                    onChange={(e) => setFormData({ ...formData, http_usuario: e.target.value })}
                    placeholder="admin"
                    autoComplete="off"
                  />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-600 uppercase mb-1">Senha Web</label>
                  <input
                    type="password"
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-primary"
                    value={formData.http_senha}
                    onChange={(e) => setFormData({ ...formData, http_senha: e.target.value })}
                    placeholder="••••••"
                    autoComplete="new-password"
                  />
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-600 uppercase mb-1">Porta HTTP override</label>
                  <input
                    type="number"
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-primary"
                    value={formData.http_porta}
                    onChange={(e) => setFormData({ ...formData, http_porta: e.target.value })}
                    placeholder="ex: 8443"
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Deixe vazio para detecção automática (8080 → 8443 → 80)</p>
                </div>
              </div>
            </div>
          </div>
