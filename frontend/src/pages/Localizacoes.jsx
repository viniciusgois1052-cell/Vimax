...
const Localizacoes = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json', ...(user?.api_token ? { 'X-API-Token': user.api_token } : {}) }, body: JSON.stringify(payload) });
            if (response.ok) { setIsModalOpen(false); fetchData(); }
            else { const d = await response.json(); alert("Erro ao salvar: " + (d.error || "Verifique os dados")); }
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Deseja realmente excluir esta localização?')) {
            try {
                const response = await fetch(`${API_URL}/localizacoes/${id}`, { method: 'DELETE', headers: user?.api_token ? { 'X-API-Token': user.api_token } : {} });
    ...
    const abrirModalDuplicar = (ids) => { setDupTargetIds(ids); setDupQuantidade(1); setIsDupModalOpen(true); };
    const executarDuplicar = async () => {
        ...
        for (const id of dupTargetIds) {
            ...
            for (let n = 1; n <= dupQuantidade; n++) {
                ...
                try { await fetch(`${API_URL}/localizacoes`, { method: 'POST', headers, body: JSON.stringify(payload) }); }
                catch (err) { console.error('Erro ao duplicar:', err); }
            }
        }
        setSelectedIds([]); setDupTargetIds([]); setIsDuplicating(false); fetchData();
    };

    const abrirModalTransferir = (ids) => { setTransTargetIds(ids); setTransEmpresaId(''); setIsTransModalOpen(true); };
    const executarTransferir = async () => {
        ...
        for (const id of transTargetIds) {
            const original = localizacoes.find(l => l.id === id);
            if (!original) continue;
            const payload = { nome: original.nome, descricao: original.descricao || '', empresa_id: parseInt(transEmpresaId) };
            try { await fetch(`${API_URL}/localizacoes/${id}`, { method: 'PUT', headers, body: JSON.stringify(payload) }); }
            catch (err) { console.error('Erro ao transferir:', err); }
        }
        setSelectedIds([]); setTransTargetIds([]); setIsTransferring(false); fetchData();
    };
    ...
    const renderEmpresaOptions = () => {
        const buildTree = (parentId = null, level = 0) => {
            return empresas.filter(e => e.parent_id === parentId).flatMap(empresa => [
    ...
    const renderEmpresaOptionsTransfer = () => {
        const lista = entities.length > 0 ? entities : empresas;
        return lista.map(e => (
            <option key={e.id} value={e.id.toString()}>
                {'\u00A0'.repeat((e.level || 0) * 4)}{(e.level || 0) > 0 ? '└─ ' : ''}{e.nome}
            </option>
        ));
    };
    ...
    const empresaDestinoNome = (entities.length > 0 ? entities : empresas).find(e => e.id.toString() === transEmpresaId)?.nome || '';

    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <FaMapMarkerAlt className="text-black" /> Gestão de Localizações
                </h1>
                <div className="flex gap-2">
                    {someSelected && (
                        <>
                            <button onClick={() => abrirModalTransferir(selectedIds)} disabled={isTransferring}
                            ...
                            <button onClick={() => abrirModalDuplicar(selectedIds)} disabled={isDuplicating}
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100">
                                ...
                                <td className="px-6 py-4">
                                    {l.empresa_id ? (
                                        <span className="text-sm text-gray-600 flex items-center gap-2">
                                            <FaBuilding className="text-indigo-400" />{buildEmpresaHierarchy(l.empresa_id)}
                                        </span>
                                    ) : (
                        ...
                        {filteredLocalizacoes.length === 0 && (
                            <tr><td colSpan={5} className="text-center py-12 text-gray-400 text-sm">Nenhuma localização encontrada.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <select value={transEmpresaId} onChange={e => setTransEmpresaId(e.target.value)}
                                    ...
                                    {renderEmpresaOptionsTransfer()}
                                </select>
                                <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                                    <FaInfoCircle /> Somente empresas do seu acesso são listadas.
                                </p>
                            </div>
                            {transEmpresaId && (
                        ...
                        <div className="p-5 bg-gray-50 border-t border-gray-200 flex gap-3">
                            <button onClick={() => { setIsTransModalOpen(false); setTransTargetIds([]); setTransEmpresaId(''); }}
                                className="flex-1 px-4 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-gray-100 transition-all">Cancelar</button>
                            <button onClick={executarTransferir} disabled={!transEmpresaId}
                                className="flex-1 px-4 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-bold shadow-lg transition-all active:scale-95 disabled:opacity-40">Transferir</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
                        ...
                        <div className="p-6">
                            ...
                            <p className="text-xs text-gray-400 mt-2 text-center">Serão criadas <strong>{dupQuantidade * dupTargetIds.length}</strong> cópia(s) no total. Máximo: 50.</p>
                        </div>
                        <div className="p-5 bg-gray-50 border-t border-gray-200 flex gap-3">
                            <button onClick={() => { setIsDupModalOpen(false); setDupTargetIds([]); }}
                                className="flex-1 px-4 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-xl font-bold hover:bg-gray-100 transition-all">Cancelar</button>
                            <button onClick={executarDuplicar}
                                className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg transition-all active:scale-95">Duplicar</button>
                        </div>
                    </div>
                </div>
            )}
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8">
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <input type="text" required className="w-full p-3 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500"
                                        value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 mb-2">Descrição</label>
                                    <textarea rows="3" className="w-full p-3 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                                        value={formData.descricao} onChange={(e) => setFormData({...formData, descricao: e.target.value})} />
                                </div>
                                <div className="bg-gray-50 p-6 rounded-xl border border-gray-100">
                                    <label className="block text-sm font-bold text-gray-700 mb-4 flex items-center gap-2">
                                        <FaBuilding className="text-black" /> Vincular Empresa
                                    </label>
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-4">
                                ...
                                <button type="submit" className="px-10 py-3 bg-black hover:bg-black text-white rounded-xl font-bold shadow-lg shadow-gray-500 transition-all">
