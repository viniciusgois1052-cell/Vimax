...
import {
    ...
    Copy, ArrowRightLeft, CheckSquare, Square, ChevronDown
} from 'lucide-react';
...
export default function Ativos() {
    ...
    const handleBulkDuplicate = async () => {
        ...
        for (const id of selectedIds) {
            ...
            const payload = {
                ...
                fornecedor_id: ativo.fornecedor_id || null, contrato_id: null, orcamento_id: null, anexos: []
            };
            try {
    ...
    const handleOpenModal = (ativo = null) => {
        if (ativo) {
            ...
            setFormData({
                nome: ativo.nome || '', numero_serie: ativo.numero_serie || '',
                voltagem_entrada: ativo.voltagem_entrada || '',
            ...
            setFormData({ nome: '', numero_serie: '', voltagem_entrada: '', data_aquisicao: '', data_inativacao: '', empresa_id: '', localizacao_id: 'none', fornecedor_id: 'none', contrato_id: 'none', orcamento_id: 'none', anexos: getDraftAnexos() || [] });
        }
        setIsModalOpen(true);
    };
    ...
    const handleDuplicar = async (ativo) => {
        const headers = { 'Content-Type': 'application/json' };
        if (user?.api_token) headers['X-API-Token'] = user.api_token;
        const payload = { nome: `${ativo.nome} (Cópia)`, numero_serie: '', voltagem_entrada: ativo.voltagem_entrada || '', data_aquisicao: ativo.data_aquisicao || '', data_inativacao: '', empresa_id: ativo.empresa_id || null, localizacao_id: ativo.localizacao_id || null, fornecedor_id: ativo.fornecedor_id || null, contrato_id: null, orcamento_id: null, anexos: [] };
        try {
            let res = await fetch(API_COLLECTION, { method: 'POST', headers, body: JSON.stringify(payload) });
            if (!res.ok) res = await fetch(API_COLLECTION_NO_SLASH, { method: 'POST', headers, body: JSON.stringify(payload) });
            if (res.ok) fetchData();
    ...
    const handleConfirmarTransfer = async () => {
        ...
        const headers = { 'Content-Type': 'application/json' };
        if (user?.api_token) headers['X-API-Token'] = user.api_token;
        const payload = { ...ativoParaTransferir, empresa_id: parseInt(transferEmpresaId), localizacao_id: transferLocalizacaoId === 'none' ? null : parseInt(transferLocalizacaoId), contrato_id: null };
        try {
            const urls = getItemUrls(ativoParaTransferir.id);
            let res = await fetch(urls[0], { method: 'PUT', headers, body: JSON.stringify(payload) });
            if (!res.ok) res = await fetch(urls[1], { method: 'PUT', headers, body: JSON.stringify(payload) });
            if (res.ok) { setIsTransferModalOpen(false); setAtivoParaTransferir(null); fetchData(); }
        } catch (e) { console.error(e); }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const payload = { ...formData, empresa_id: formData.empresa_id ? parseInt(formData.empresa_id) : null, localizacao_id: formData.localizacao_id === 'none' ? null : parseInt(formData.localizacao_id), fornecedor_id: formData.fornecedor_id === 'none' ? null : parseInt(formData.fornecedor_id), contrato_id: formData.contrato_id === 'none' ? null : parseInt(formData.contrato_id), orcamento_id: formData.orcamento_id === 'none' ? null : parseInt(formData.orcamento_id), anexos: formData.anexos };
        const headers = { 'Content-Type': 'application/json' };
        if (user?.api_token) headers['X-API-Token'] = user.api_token;
    ...
    const filteredAtivos = useMemo(() => ativos.filter(a => {
        const matchesSearch = a.nome.toLowerCase().includes(searchTerm.toLowerCase()) || (a.numero_serie && a.numero_serie.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className={`mb-4 flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-200 ${someSelected ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-gray-200'}`}>
                {/* Checkbox selecionar tudo */}
                <button onClick={toggleSelectAll} className="flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-indigo-600 transition-colors">
                    {allSelected
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAtivos.map(a => {
                    ...
                    return (
                        <Card key={a.id}
                            className={`transition-all border-2 group relative overflow-hidden cursor-pointer ${isSelected ? 'border-indigo-400 shadow-md ring-2 ring-indigo-200' : 'border-gray-200 hover:shadow-md'}`}>
                            <CardContent className="p-0">
                                        ...
                                        <div className="flex items-start gap-2 flex-1 min-w-0">
                                            <button onClick={() => toggleSelect(a.id)}
                                                ...
                                                {isSelected
                                                    ...
                                                    : <Square size={18} className="text-gray-300 hover:text-indigo-400" />}
                                            </button>
                                            <h3 className="font-bold text-gray-800 text-base flex items-center gap-1.5 truncate">
                                                <Box size={16} className="text-indigo-500 shrink-0" /> {a.nome}
                                            </h3>
                                        </div>
                                        {/* Ações individuais */}
                                        <div className="flex gap-1 shrink-0 ml-1">
                                            <button onClick={() => handleOpenQRCode(a)} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors" title="QR Code"><QrCode size={15} /></button>
                                            ...
                                            <button onClick={() => handleOpenModal(a)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Editar"><Edit2 size={15} /></button>
                                            <button onClick={() => handleDelete(a.id)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Excluir"><Trash2 size={15} /></button>
                                        </div>
                                    </div>
                                ...
                                <div className={`p-4 flex justify-between items-center transition-colors ${isSelected ? 'bg-indigo-50/60' : 'bg-gray-50/50'}`}>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                        ...
                        <div className="p-5 border-t flex gap-3">
                            ...
                            <button onClick={handleBulkTransferConfirm}
                                ...
                                className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <p className="text-xs text-gray-400">⚠️ O contrato vinculado será desassociado.</p>
                        </div>
                        <div className="p-5 border-t flex gap-3">
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
                        ...
                        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-2">
                            {attachmentsToShow.map((anexo, idx) => (
                                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-indigo-200">
                                    ...
                                    <button onClick={() => window.open(getAnexoHref(anexo.path || anexo.url), '_blank')} className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg"><Eye size={18} /></button>
                                </div>
                            ))}
                        </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden p-6 text-center space-y-4">
                        ...
                        <div className="flex gap-3">
                            <button onClick={handlePrintQRCode} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-lg font-bold flex items-center justify-center gap-2 shadow-md transition-all">
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" required className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.nome} onChange={e => setFormData({...formData, nome: e.target.value})} />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Nº Série</label>
                                            <input type="text" className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.numero_serie} onChange={e => setFormData({...formData, numero_serie: e.target.value})} />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Voltagem</label>
                                            <input type="text" placeholder="220V" className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.voltagem_entrada} onChange={e => setFormData({...formData, voltagem_entrada: e.target.value})} />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Aquisição</label>
                                            <input type="date" className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.data_aquisicao} onChange={e => setFormData({...formData, data_aquisicao: e.target.value})} />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Inativação</label>
                                            <input type="date" className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.data_inativacao} onChange={e => setFormData({...formData, data_inativacao: e.target.value})} />
                                        </div>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select required className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.empresa_id} onChange={e => setFormData({...formData, empresa_id: e.target.value})}>
                                            <option value="">Selecione...</option>
                                            {renderHierarchicalOptions(empresas)}
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <select className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500" value={formData.localizacao_id} onChange={e => setFormData({...formData, localizacao_id: e.target.value})}>
                                            <option value="none">Nenhuma</option>
                                            {renderGroupedLocalizacoes()}
                                        </select>
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 text-sm" value={formData.fornecedor_id} onChange={e => setFormData({...formData, fornecedor_id: e.target.value})}>
                                        ...
                                        <div>
                                            ...
                                            <select className="w-full p-2.5 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 text-sm" value={formData.contrato_id} onChange={e => setFormData({...formData, contrato_id: e.target.value})}>
                                                <option value="none">Nenhum</option>
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                ...
                                <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-indigo-300 transition-colors relative group">
                                    ...
                                    <Paperclip className="mx-auto text-gray-400 group-hover:text-indigo-500 mb-2" size={24} />
                                    ...
                                    {formData.anexos.map((file, idx) => (
                                        <div key={idx} className="flex items-center justify-between bg-gray-50 p-3 rounded-xl border border-gray-100 group">
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button type="button" onClick={() => window.open(getAnexoHref(file.path || file.url), '_blank')} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md"><Eye size={16} /></button>
                            ...
                            <div className="pt-6 border-t flex justify-end gap-4">
                                ...
                                <button type="submit" disabled={uploading} className="px-12 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg disabled:opacity-50">
