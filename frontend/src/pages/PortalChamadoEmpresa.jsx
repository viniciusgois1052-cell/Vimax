...
import { Wrench, AlertCircle, CheckCircle, Camera, X, ChevronLeft, Send, Loader2, Layers, MapPin, Monitor } from 'lucide-react'

const API_BASE = ""
const API = `${API_BASE}/api`

const VIMAX_LOGO = 'http://wiki.digimaxdiagnostico.com.br/wp/wp-content/uploads/2026/01/Vimax-Icone.png'
const GLPI_URL = 'http://glpi.ad.digimaxdiagnostico.com.br:5000'
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4" style={{ background: '#efefef' }}>
            <div className="w-full max-w-lg">
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    {/* Botão T.I — abre o GLPI em nova aba */}
                    <button onClick={() => window.open(GLPI_URL, '_blank', 'noopener,noreferrer')}
                        className="p-6 text-left transition-all duration-200 border"
                        style={{ background: '#fff', borderColor: '#e5e7eb', borderRadius: '1.5rem' }}
                        onMouseEnter={e => e.currentTarget.style.borderColor = '#d1d5db'}
                        onMouseLeave={e => e.currentTarget.style.borderColor = '#e5e7eb'}>
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 flex items-center justify-center border" style={{ background: '#f9fafb', borderColor: '#e5e7eb', borderRadius: '1rem' }}>
                                <Monitor size={26} style={{ color: '#6b7280' }} />
                            </div>
                            <div>
                                <h3 className="text-lg font-bold mb-1" style={{ color: '#374151' }}>T.I</h3>
                                <p className="text-sm" style={{ color: '#9ca3af' }}>Chamados de suporte técnico</p>
                            </div>
                        </div>
                    </button>
                </div>
...
// ── Ao trocar localização, limpa item e opções ────────────────────────────
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            titulo: `[${isMaquinario ? 'Maquinário' : 'Infraestrutura'}] ${nomeItem}: ${tituloBase}`.substring(0, 200),
...
export default function PortalChamadoEmpresa() {
    ...
    const handleLoginSuccess = async (portalUser, portalToken) => {
        try {
            const [resAtivos, resInfras, resLocs, resForms] = await Promise.all([
                ...
                fetch(`${API}/formularios-chamado`, { headers: portalToken ? {'X-API-Token': portalToken} : {} })
            ])
            if (resAtivos.ok) {
