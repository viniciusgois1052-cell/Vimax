...
import { Plus, Edit2, Trash2, X, Eye, FileText, Code, StickyNote } from 'lucide-react'
...
export default function MarketingModelos() {
  ...
  const salvar = async () => {
    if (!form.nome || !form.assunto || !form.corpo_html) return alert('Preencha nome, assunto e conteúdo!')
    setLoading(true)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Crie templates HTML reutilizáveis para suas campanhas</p>
        </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-2">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col" style={{height:'95vh'}}>

            <div className="p-5 bg-gradient-to-r from-primary to-primary/80 text-white flex items-center justify-between shrink-0">
            ...
            <div className="px-6 pt-4 pb-2 grid grid-cols-2 gap-4 shrink-0">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-primary text-sm"
                  placeholder="Ex: Novidades do mês!"
                />
              </div>
            </div>
              ...
              <div className="flex border-b border-slate-200">
                ...
                {aba === 'html' && (
                  <div className="ml-auto flex items-center gap-2 py-2">
                    <span className="text-xs font-bold text-slate-400">Variáveis:</span>
                    {['{{nome}}','{{email}}'].map(v => (
                      <button key={v} onClick={() => inserirVariavel(v)}
                        className="px-2 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-lg hover:bg-indigo-100 border border-indigo-200">
                        {v}
                      </button>
                    ))}
                  </div>
            ...
            <div className="flex-1 overflow-hidden relative">
              ...
              {aba === 'html' && (
                <div className="relative h-full">
                  <textarea
                    ...
                    className="w-full h-full p-4 font-mono text-sm outline-none resize-none bg-slate-900 text-green-400"
              ...
              {aba === 'preview' && (
                <div className="bg-slate-100 p-4 h-full overflow-auto">
                  <div className="bg-white rounded-xl shadow-sm h-full overflow-auto">
                    {form.corpo_html ? (
                      ...
                      <div className="flex items-center justify-center h-full text-slate-400">
                        <p className="text-sm">Nenhum conteúdo. Escreva o HTML na aba Editor HTML.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {aba === 'notas' && (
                <div className="flex flex-col h-full bg-white overflow-y-auto">
                  <div className="flex items-center gap-1 border-b border-slate-200 px-3 py-2 bg-slate-50 flex-wrap shrink-0">
                    {[
                      ['<b>','</b>','B','bold','Negrito'],
                  ...
                  <textarea
                    ...
                    className="flex-1 p-4 text-sm text-slate-700 outline-none resize-none font-mono border-0"
