        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                    <Wrench className="text-black" /> Gestão de Infraestrutura
                </h1>
                {can('infraestrutura','criar') && (
            ...
            <div className={`mb-4 flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-200 ${someSelected ? 'bg-orange-50 border-orange-200 shadow-sm' : 'bg-white border-gray-200'}`}>
                <button onClick={toggleSelectAll} className="flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-black transition-colors">
                    {allSelected
                            ...
                            <CardContent className="p-0">
                                <div className="p-5 border-b border-gray-100 bg-white">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="flex items-start gap-2 flex-1 min-w-0">
                                            ...
                                            <h3 className="font-bold text-gray-800 text-base flex items-center gap-1.5 truncate">
                                                <Wrench size={15} className="text-gray-500 shrink-0" /> {i.nome}
                                            </h3>
                                        </div>
                                        {/* Ações individuais */}
                                    ...
                                    <div className="space-y-1.5 text-sm text-gray-600 pl-6">
                                        {i.tipo_nome && <p className="flex items-center gap-2"><Wrench size={13} className="text-gray-400" /> Tipo: <span className="font-semibold text-gray-500">{i.tipo_nome}</span></p>}
                        ...
                        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-2">
                            {attachmentsToShow.map((anexo, idx) => (
                                <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-orange-200">
                                    ...
                                    <button onClick={() => openSecureFile(anexo.path || anexo.url)}
                                    ...
                                    {formData.anexos.map((file, idx) => (
                                        <div key={idx} className="flex items-center justify-between bg-gray-50 p-3 rounded-xl border border-gray-100 group">
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button type="button" onClick={() => openSecureFile(file.path || file.url)} className="p-1.5 text-orange-600 hover:bg-orange-50 rounded-md"><Eye size={16} /></button>
