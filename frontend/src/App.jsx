...
import { MapPin, Users, FileText, DollarSign, Wrench, User, Mail, BarChart, Building2, Menu, X, Box, LogOut, Lock, Tag, Hammer, AlertTriangle, Bell, Send, Wifi, Database, Printer, TrendingUp, UserCheck, ShieldCheck, StickyNote, ShoppingCart, ChevronDown, Image, FolderKanban } from 'lucide-react'
import './App.css'
...
import MarketingNotas from './pages/MarketingNotas'
import Logs from './pages/Logs'
import PerfilAcesso from './pages/PerfilAcesso'
import CRM from './pages/CRM/CRM'
import Mobilemed from './pages/Mobilemed'
import Clientes from './pages/Clientes'
import ContadoresImpressora from './pages/ContadoresImpressora'

// 🆕 COMPRAS
import Compras from './pages/Compras'
import GruposItens from './pages/compras/GruposItens'
import ItensEstrutura from './pages/compras/ItensEstrutura'
import RequisicaoCompra from './pages/compras/RequisicaoCompra'
import PedidoCompra from './pages/compras/PedidoCompra'
import OrdemCompra from './pages/compras/OrdemCompra'
import OrcamentosCompras from './pages/compras/OrcamentosCompras'
import Recebimento from './pages/compras/Recebimento'
import RelatorioCompras from './pages/compras/RelatorioCompras'
import PortalFornecedor from './pages/compras/PortalFornecedor'
import ConfigLogo from './pages/compras/ConfigLogo'
import DocumentosFornecedor from './pages/compras/DocumentosFornecedor'

// GESTÃO DE PROJETOS
import Projeto from './pages/projeto'
import ProjetoHub from './pages/projeto/ProjetoHub'
import ProjetoProjecoes from './pages/projeto/Projecoes'

const SUPER_ADMIN_ONLY   = ['super_admin']
const ADMIN_ROLES        = ['super_admin', 'admin']
const MARKETING_ROLES    = ['super_admin', 'marketing']
const ALL_INTERNAL       = ['super_admin', 'admin', 'relatorios', 'self_service']
const DOCS_ROLES         = ['super_admin', 'admin', 'gestao_documentos']
const DOCS_CLIENTES      = ['super_admin', 'gestao_documentos']
const DOCS_LEMBRETE      = ['super_admin', 'admin', 'relatorios', 'gestao_documentos']
const COMPRAS_ROLES      = ['super_admin']
const PROJETOS_ROLES     = ['super_admin', 'admin']
...
function Navigation({ isCollapsed, toggleCollapse }) {
  ...
  // 🆕 controla quais grupos do menu estão abertos/fechados (modo expandido)
  // undefined ou true = aberto, false = fechado
  ...
  // 🆕 controla o flyout de grupo no modo recolhido (hover)
  const [hoveredFlyout, setHoveredFlyout] = useState(null) // { top, title, items }
  const flyoutTimeout = useRef(null)

  const handleGroupEnter = (e, group, items) => {
    clearTimeout(flyoutTimeout.current)
    const rect = e.currentTarget.getBoundingClientRect()
    setHoveredFlyout({ top: rect.top, title: group.title, items })
  }
  const handleGroupLeave = () => {
    flyoutTimeout.current = setTimeout(() => setHoveredFlyout(null), 150)
  }
  const handleFlyoutEnter = () => clearTimeout(flyoutTimeout.current)
  const handleFlyoutLeave = () => setHoveredFlyout(null)
  ...
  const menuGroups = [
    ...
    },
    {
      ...
      groupIcon: FileText,
      items: [
        { path: '/contratos',   icon: FileText,     label: 'Contratos',      roles: DOCS_ROLES },
        { path: '/orcamentos',  icon: DollarSign,   label: 'Orcamentos',     roles: ADMIN_ROLES },
        { path: '/clientes',    icon: UserCheck,    label: 'Clientes',       roles: DOCS_CLIENTES },
        { path: '/lembretes',   icon: Bell,         label: 'Meus Lembretes', roles: DOCS_LEMBRETE },
      ]
    },
    {
      ...
      items: [
        ...
        { path: '/fornecedores',           icon: Users,     label: 'Fornecedores / Prestadores',          roles: ADMIN_ROLES },
    ...
    },
  {
    ...
    items: [
      { path: '/compras', icon: ShoppingCart, label: 'Compras', roles: COMPRAS_ROLES },
    ]
  },
    {
      ...
      items: [
        { path: '/projetos', icon: FolderKanban, label: 'Projetos', roles: PROJETOS_ROLES },
      ]
    },
    {
      title: 'BI',
    ...
    },
    {
      ...
      items: [
        ...
        { path: '/marketing/notas',     icon: StickyNote, label: 'Notas',        roles: MARKETING_ROLES },
      ]
    },
    {
      title: 'Configurações',
      ...
      items: [
        { path: '/mobilemed',     icon: Database,    label: 'Mobilemed API',    roles: SUPER_ADMIN_ONLY },
        { path: '/usuarios',      icon: User,        label: 'Usuarios',         roles: SUPER_ADMIN_ONLY },
        { path: '/config-email',  icon: Mail,        label: 'Config. Email',    roles: SUPER_ADMIN_ONLY },
        { path: '/logs',          icon: FileText,    label: 'Logs',             roles: SUPER_ADMIN_ONLY },
        { path: '/perfis-acesso', icon: ShieldCheck, label: 'Perfis de Acesso', roles: SUPER_ADMIN_ONLY },
      ]
    }
  ]
  ...
  return (
    <nav className={`bg-card border-r border-border h-screen fixed left-0 top-0 flex flex-col transition-all duration-300 z-50 ${sidebarWidth}`}>
      ...
      {!isCollapsed && !isSelfService && (treeEntities || []).length > 0 && (
        <div className="p-4 border-b border-border">
          <p className="text-[10px] text-muted-foreground mb-4 text-center uppercase tracking-widest font-bold"> </p>
      ...
      {!isCollapsed && isMarketing && !user.perfil_acesso && user.empresa_nome && (
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            ...
            {!isCollapsed && (
              <p className="text-[10px] text-muted-foreground font-bold uppercase">
                {user.perfil_acesso
                  ...
                  : user.role === 'gestao_documentos' ? 'Gestão Documentos'
                  : user.role.replace('_', ' ')}
              </p>
            )}
          </div>
        ...
        <div className="space-y-6">
          {menuGroups.map((group, groupIdx) => {
            const p = user?.perfil_acesso
            const filteredItems = group.items.filter(item => {
              if (!user) return false
              if (user.role === 'super_admin') return true
              if (p) {
                const modMap = {
                  '/chamados':               !!p.chamados_ver,
                  '/categorias-chamado':     !!p.tipo_chamado_ver,
                  '/tipos-servico':          !!p.tipo_servico_ver,
                  '/formularios-chamado':    !!p.formularios_chamado_ver,
                  '/contratos':              !!p.contratos_ver,
                  '/orcamentos':             !!p.orcamentos_ver,
                  '/compras':                !!p.compras_ver,
                  '/compras/logo':           !!p.compras_ver,
                  '/projetos':               !!p.projetos_ver,
                  '/clientes':               !!p.clientes_ver,
                  '/lembretes':              !!p.lembretes_ver,
                  '/empresas':               !!p.empresas_ver,
                  '/localizacoes':           !!p.localizacoes_ver,
                  '/ativos':                 !!p.ativos_ver,
                  '/fornecedores':           !!p.fornecedores_ver,
                  '/tipos-infraestrutura':   !!p.tipo_infraestrutura_ver,
                  '/infraestruturas':        !!p.infraestrutura_ver,
                  '/contadores-impressora':  !!p.contadores_impressora_ver,
                  '/relatorios':             !!p.relatorios_ver,
                  '/crm':                    !!p.crm_ver,
                  '/marketing/contatos':     !!p.marketing_ver,
                  '/marketing/grupos':       !!p.marketing_ver,
                  '/marketing/smtp':         !!p.marketing_ver,
                  '/marketing/modelos':      !!p.marketing_ver,
                  '/marketing/campanhas':    !!p.marketing_ver,
                  '/marketing/notas':        !!p.marketing_ver,
                  '/usuarios':               !!p.usuarios_ver,
                  '/perfis-acesso':          !!p.perfis_acesso_ver,
                  '/config-email':           !!p.config_email_ver,
                  '/logs':                   !!p.logs_ver,
                  '/mobilemed':              !!p.mobilemed_ver,
                }
                if (item.path in modMap) return modMap[item.path]
                return false
              }
            ...
            // 🆕 MODO RECOLHIDO: um ícone só por grupo, com flyout no hover
            if (isCollapsed) {
              const GroupIcon = group.groupIcon || filteredItems[0].icon
              const groupIsActive = filteredItems.some(item => location.pathname.startsWith(item.path))
              return (
                <div key={groupIdx}>
                  <button
                    onMouseEnter={(e) => handleGroupEnter(e, group, filteredItems)}
            ...
            // MODO EXPANDIDO: acordeão (título clicável + lista de itens)
            ...
            return (
              <div key={groupIdx} className="space-y-2">
                ...
                >
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 border-b border-border/50 pb-1 flex-1 text-left">
                    {group.title}
                  </p>
                  <ChevronDown
                    className={`w-3.5 h-3.5 text-muted-foreground/60 transition-transform duration-200 ${
                      openGroups[group.title] === false ? '-rotate-90' : ''
                    }`}
                  />
                </button>
                {isOpen && (
      ...
      {/* 🆕 FLYOUT do grupo (modo recolhido) — fixed para não ser cortado pelo overflow da sidebar */}
      {isCollapsed && hoveredFlyout && (
        <div
          className="fixed z-[60] w-64 bg-card border border-border rounded-2xl shadow-2xl p-3"
          style={{
            left: '5.5rem',
            top: Math.min(Math.max(hoveredFlyout.top, 12), window.innerHeight - 24 - hoveredFlyout.items.length * 44)
          }}
...
function AppContent() {
  ...
  useEffect(() => {
    if (user?.api_token && user?.id) refreshUser()
  }, [])
  ...
  const isPortalRoute = window.location.pathname.match(/^\/portal\/\d+$/) || window.location.pathname.startsWith('/portal-fornecedor')
  ...
  const isGestaoDocumentos = role === 'gestao_documentos'
  const p                  = user.perfil_acesso

  const isCRMRoute      = location.pathname.startsWith('/crm')
  const isProjetosRoute = location.pathname.startsWith('/projetos')
  const mainMargin  = isPublicUser ? "ml-0" : (isCollapsed ? "ml-20" : "ml-64")
  const mainPadding = (isCRMRoute || isProjetosRoute) ? 'p-0' : 'p-8'

  const firstAllowedRoute = () => {
    if (!p) return '/chamados'
    if (p.chamados_ver)   return '/chamados'
    if (p.contratos_ver)  return '/contratos'
    if (p.orcamentos_ver) return '/orcamentos'
    if (p.clientes_ver)   return '/clientes'
    if (p.projetos_ver)   return '/projetos'
    if (p.ativos_ver)     return '/ativos'
    if (p.relatorios_ver) return '/relatorios'
    if (p.crm_ver)        return '/crm'
    if (p.marketing_ver)  return '/marketing/campanhas'
    if (p.usuarios_ver)   return '/usuarios'
    return '/chamados'
  }
  ...
  const renderRoutes = () => {
    ...
    // ✅ QUALQUER ROLE COM PERFIL CUSTOMIZADO — o perfil manda, o role é ignorado
    if (p) {
      return (
        <Routes>
          <Route path="/portal/:empresa_id" element={<PortalChamadoEmpresa />} />
          ...
          {p.chamados_ver            && <Route path="/chamados"               element={<Chamados />} />}
          {p.tipo_chamado_ver        && <Route path="/categorias-chamado"     element={<CategoriasChamado />} />}
          {p.tipo_servico_ver        && <Route path="/tipos-servico"          element={<TipoServico />} />}
          {p.formularios_chamado_ver && <Route path="/formularios-chamado"    element={<FormularioChamadoAdmin />} />}

          {p.contratos_ver           && <Route path="/contratos"             element={<Contratos />} />}
          {p.orcamentos_ver          && <Route path="/orcamentos"            element={<Orcamentos />} />}
          {p.clientes_ver            && <Route path="/clientes"              element={<Clientes />} />}
          {p.lembretes_ver           && <Route path="/lembretes"             element={<Lembretes />} />}

          {p.compras_ver && <>
            <Route path="/compras"                element={<Compras />} />
            <Route path="/compras/grupos"         element={<GruposItens />} />
            <Route path="/compras/itens"          element={<ItensEstrutura />} />
            <Route path="/compras/requisicoes"    element={<RequisicaoCompra />} />
            <Route path="/compras/pedidos"        element={<PedidoCompra />} />
            <Route path="/compras/ordens"         element={<OrdemCompra />} />
            <Route path="/compras/orcamentos"     element={<OrcamentosCompras />} />
            <Route path="/compras/recebimento"    element={<Recebimento />} />
            <Route path="/compras/relatorios"     element={<RelatorioCompras />} />
            <Route path="/compras/logo"           element={<ConfigLogo />} />
            <Route path="/compras/documentos-fornecedor" element={<DocumentosFornecedor />} />
          </>}

          {p.projetos_ver && <>
            <Route path="/projetos" element={<ProjetoHub />} />
            <Route path="/projetos/gestao" element={<Projeto />} />
            <Route path="/projetos/projecoes" element={<ProjetoProjecoes />} />
          </>}

          {p.empresas_ver              && <Route path="/empresas"              element={<Empresas />} />}
          {p.localizacoes_ver          && <Route path="/localizacoes"          element={<Localizacoes />} />}
          {p.ativos_ver                && <Route path="/ativos"                element={<Ativos />} />}
          {(p.fornecedores_ver || p.compras_editar) && <Route path="/fornecedores" element={<Fornecedores />} />}
          {p.tipo_infraestrutura_ver   && <Route path="/tipos-infraestrutura"  element={<TipoInfraestrutura />} />}
          {p.infraestrutura_ver        && <Route path="/infraestruturas"       element={<Infraestrutura />} />}
          {p.contadores_impressora_ver && <Route path="/contadores-impressora" element={<ContadoresImpressora />} />}

          {p.relatorios_ver && <Route path="/relatorios" element={<Relatorios />} />}

          {p.crm_ver && <Route path="/crm/*" element={<CRM />} />}
          ...
          {p.marketing_ver && <>
            ...
            <Route path="/marketing/notas"     element={<MarketingNotas />} />
          </>}

          {p.usuarios_ver      && <Route path="/usuarios"      element={<Usuarios />} />}
          {p.perfis_acesso_ver && <Route path="/perfis-acesso" element={<PerfilAcesso />} />}
          {p.config_email_ver  && <Route path="/config-email"  element={<ConfigEmail />} />}
          {p.logs_ver          && <Route path="/logs"          element={<Logs />} />}
          {p.mobilemed_ver     && <Route path="/mobilemed"     element={<Mobilemed />} />}

          <Route path="*" element={<AcessoRestrito mensagem="Você não tem permissão para acessar esta página." />} />
        </Routes>
      )
    }
    ...
    if (isAdmin) {
      return (
        <Routes>
          ...
          <Route path="/contadores-impressora"     element={<ContadoresImpressora />} />
          <Route path="/relatorios"                element={<Relatorios />} />
          <Route path="*"                          element={<Navigate to="/chamados" replace />} />
        </Routes>
      )
    ...
    if (isSuperAdmin) {
      return (
        <Routes>
          ...
          <Route path="/marketing/campanhas"       element={<MarketingCampanhas />} />
          <Route path="/marketing/notas"           element={<MarketingNotas />} />
          <Route path="/mobilemed"                 element={<Mobilemed />} />
          <Route path="/usuarios"                  element={<Usuarios />} />
          <Route path="/config-email"              element={<ConfigEmail />} />
          <Route path="/logs"                      element={<Logs />} />
          <Route path="/perfis-acesso"             element={<PerfilAcesso />} />
          <Route path="*"                          element={<Navigate to="/chamados" replace />} />
        </Routes>
      )
    }
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className={`flex-1 transition-all duration-300 ${mainMargin} ${mainPadding} ${isCRMRoute ? "p-0 flex flex-col overflow-hidden" : ""}`} style={isCRMRoute ? {height: "100vh", overflow: "hidden"} : {}}>
        {user && isPublicUser && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden">
            ...
            <div className="flex border-b border-slate-200 bg-slate-50">
              <button onClick={() => setAbaModal('contratos')}
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold transition-all border-b-2 ${abaModal === 'contratos' ? 'border-red-500 text-red-600 bg-white' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}>
              ...
              <button onClick={() => setAbaModal('lembretes')}
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold transition-all border-b-2 ${abaModal === 'lembretes' ? 'border-amber-500 text-amber-600 bg-white' : 'border-transparent text-slate-500 hover:bg-slate-100'}`}>
