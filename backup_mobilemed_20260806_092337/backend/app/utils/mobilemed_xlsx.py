...
FIELD_ALIASES = {
    ...
    "empresa.nome_fantasia": [
        "empresa.nome_fantasia", "nome_fantasia", "nome fantasia", "empresa",
    ],
    "usuario.nome": [
...
def _find_actual_header(campo: str, cabecalhos: list[str]) -> str | None:
    mapa = {_normalize_column(c): c for c in cabecalhos}
    candidatos = [
        campo,
        campo.replace(".", "_"),
        campo.split(".")[-1],
    ...
    for candidato in candidatos:
        encontrado = mapa.get(_normalize_column(candidato))
        if encontrado:
            return encontrado
...
def _project_rows(
    ...
    mapeamento = {
        campo: _find_actual_header(campo, cabecalhos_csv)
        for campo in campos
    }
