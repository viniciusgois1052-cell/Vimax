...
def get_custos_contratos():
    ...
    try:
        with urllib.request.urlopen(
            ...
            cotacao_usd = float(
                json.loads(resposta.read())['USDBRL']['bid']
            )
    except Exception:
        cotacao_usd = 5.0
    ...
    for contrato in contratos:
        valor = float(contrato.valor or 0)
        moeda = (contrato.moeda or 'BRL').upper()

        if contrato.is_mensal:
            valor_mensal_brl = valor
        elif contrato.data_inicio and contrato.data_fim:
            meses = max(
                1,
                (contrato.data_fim.year - contrato.data_inicio.year) * 12
                + contrato.data_fim.month
                - contrato.data_inicio.month
            )
            valor_mensal_brl = valor / meses
        else:
            valor_mensal_brl = valor

        if moeda == 'USD':
            valor_mensal_brl *= cotacao_usd

        valor_anual_acumulado = valor_mensal_brl * mes_atual
        ativos = [
            ativo
            for ativo in (contrato.ativos_vinculados or [])
            if ativo.empresa_id == contrato.empresa_id
        ]

        empresa_nome = (
            contrato.empresa.nome if contrato.empresa else 'Sem Empresa'
        )
        empresa_id = contrato.empresa_id or 0

        def adicionar_item(tipo, ativo=None):
            ...
            empresas = resultado[tipo]['empresas']
            ...
            item = {
                ...
                'numero_serie': (
                    (ativo.numero_serie or '') if ativo else ''
                ),
                ...
                'valor_anual_acumulado': round(
                    valor_anual_acumulado,
                    2
                ),
                'is_mensal': bool(contrato.is_mensal),
            }
            empresas[empresa_key]['ativos'].append(item)
            empresas[empresa_key]['total_mensal'] += valor_mensal_brl
            empresas[empresa_key]['total_anual'] += valor_anual_acumulado
            resultado[tipo]['total_mensal'] += valor_mensal_brl
            resultado[tipo]['total_anual'] += valor_anual_acumulado

        if ativos:
            for ativo in ativos:
                adicionar_item(ativo.nome or 'Sem Tipo', ativo)
        else:
            adicionar_item('Sem Ativo Vinculado')

    grupos = []
    total_geral_mensal = 0.0
    total_geral_anual = 0.0
    for grupo in resultado.values():
        grupo['empresas'] = list(grupo['empresas'].values())
        grupo['total_mensal'] = round(grupo['total_mensal'], 2)
        grupo['total_anual'] = round(grupo['total_anual'], 2)
        total_geral_mensal += grupo['total_mensal']
        total_geral_anual += grupo['total_anual']
        grupos.append(grupo)
